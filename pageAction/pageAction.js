...
(async function(){
	...
	extension.message.on(function(data){
		if (data["antiFingerprinting-notificationCounter"]){
			const url = new URL(data.url);
			Object.keys(data["antiFingerprinting-notificationCounter"]).forEach(function(key){
				domainNotification(
					url,
					key,
					data["antiFingerprinting-notificationCounter"][key].count,
					data["antiFingerprinting-notificationCounter"][key].api
				);
			});
		}
		if (
			Array.isArray(data["antiFingerprinting-notifications"]) &&
			data["antiFingerprinting-notifications"].length
		){
			logging.message("got notifications");
			const notifications = data["antiFingerprinting-notifications"];
			let i = 0;
	...
	browser.tabs.sendMessage(
		tab.id,
		{
			"antiFingerprinting-sendNotifications": tab.id
		}
	);
