...
(function(){
	...
	const getPersistentRnd = (function(){
		...
		extension.message.on(function(data){
			if (data["antiFingerprinting-set-domain-rnd"]){
				const {domain, incognito, rnd} = data["antiFingerprinting-set-domain-rnd"];
			...
			if (data["antiFingerprinting-clear-domain-rnd"]){
				...
				let ownPersistendRnd = data["antiFingerprinting-clear-domain-rnd"] !== "force"? persistentRnd[domain]: false;
				persistentRnd = Object.create(null);
				if (ownPersistendRnd){
					persistentRnd[domain] = ownPersistendRnd;
					extension.message.send({
						"antiFingerprinting-new-domain-rnd": {
							domain,
