...
(function(require){
	...
	if (window === window.top){
		...
		port.postMessage({"antiFingerprinting-clear-page-action": true});
	}
	let tabId;
	...
	function notify(data){
		if (!settings.ignoredAPIs[data.api]){
			...
			if (!sentAPIs[data.api]){
				...
				port.postMessage({"antiFingerprinting-notify": data});
			}
		}
	}
	...
	function interceptWindow(window){
		...
		const antiFingerprintingData = wrappedWindow.matchMedia(extensionSecret[0]);
		if (antiFingerprintingData.secret === extensionSecret[1]){
			if (wrappedWindow.top === wrappedWindow){
				antiFingerprintingData.undoIntercept(extension.extensionID);
			}
			else {
	...
	extension.message.on(function(data){
		if (data["antiFingerprinting-unload"]){
			extension.revertProperties(window);
		...
		if (
			data.hasOwnProperty("antiFingerprinting-sendNotifications") &&
			data["antiFingerprinting-sendNotifications"] === tabId
		){
			logging.notice("sending notifications:", notifications);
			extension.message.send({
				sender: tabId,
				url: window.location.href,
				"antiFingerprinting-notificationCounter": notificationCounter,
				"antiFingerprinting-notifications": notifications
			});
