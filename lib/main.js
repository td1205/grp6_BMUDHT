...
(function(){
				...
				js: [{
					code: `(function(settingsData){
						if (typeof require !== "undefined"){
							...
							console.warn(
								"[Anti Fingerprinting] invalid content script order: require not defined at",
	...
	settings.onloaded(function(){
		...
		browser.runtime.onMessage.addListener(async function(data){
			...
			if (data["antiFingerprinting-new-domain-rnd"]){
				persistentRndStorage.setDomainData(
					data["antiFingerprinting-new-domain-rnd"].domain,
					data["antiFingerprinting-new-domain-rnd"].incognito,
					data["antiFingerprinting-new-domain-rnd"].rnd
				);
				if (keys.length === 1){
					return;
				}
			}
			if (data["antiFingerprinting-clear-domain-rnd"]){
				persistentRndStorage.clear(data["antiFingerprinting-clear-domain-rnd"] === "force");
				if (keys.length === 1){
					return;
				}
			}
			if (data["antiFingerprinting-clear-container-rnd"]){
				persistentRndStorage.clearContainerData(data["antiFingerprinting-clear-container-rnd"]);
				if (keys.length === 1){
					return;
				}
			}
		...
		browser.runtime.onConnect.addListener(function(port){
			...
			port.onMessage.addListener(function(data){
				if (data.hasOwnProperty("antiFingerprinting-notify")){
					notification.show(port.sender.tab.id, url, data["antiFingerprinting-notify"].api);
				}
				if (data.hasOwnProperty("antiFingerprinting-clear-page-action")){
	...
	browser.runtime.onInstalled.addListener(function(details){
		...
		switch (details.reason){
			case "install":
				logging.message("Anti Fingerprinting installed");
			...
			case "update":
				settings.onloaded(function(){
					if (!settings.dontShowOptionsOnUpdate){
						logging.message("Anti Fingerprinting updated");
						openOptions(details.reason);
					}
				});
		}
	...
	if (browser.runtime.onSuspend){
		browser.runtime.onSuspend.addListener(async function(){
			logging.message("Suspending Anti Fingerprinting");
			(await browser.tabs.query({})).forEach(function(tab){
				browser.tabs.sendMessage(tab.id, {
					"antiFingerprinting-unload": true
				});
			});
		});
	}
