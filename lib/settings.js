...
(function(require){
	...
	if (require.exists("./settingsData")){
		...
		scope.loaded = browser.storage.local.get().then(function(res){
			if (!res.antiFingerprintingForcedReset2) {
				logging.message("Forcing initial reset of browser storage to apply maximum security defaults");
				browser.storage.local.clear();
				browser.storage.local.set({ antiFingerprintingForcedReset2: true });
				return scope.init({});
			}
			return scope.init(res);
		});
	}
